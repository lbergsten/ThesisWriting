...
\caption{Cutflow in the top validation region.}
\label{tab:topcr}
\end{table}
...
Data and MC show good agreement over various variable distributions as shown in Figure~\ref{fig:WWCR3}.
\begin{figure}[!h]
\centering
%  \subfloat[$m_{\ell\ell}$]{
...
%  }%\hfill 
{\caption{Distributions of $\Delta Y_{\ell\ell}$, $\Delta Y_{jj}$, $\sum$ centralities, $M_{l0j0}$, $M_{l1j1}$, $p_T^{j0}$, $\Delta\phi_{jj}$, $p^T_{tot}$, and $m_T$ in the $WW$ validation region. Uncertainties shown (yellow) are purely statistical.
\label{fig:WWCR3}}}
\end{figure}
%
...
The ggF CR2 BDT discriminant is trained using all ggF CR2 selection criteria. The training includes about 2,000 un-weighted (300 weighted) ggF events trained against 90,000 un-weighted (1,500 weighted) other samples. The optimal training parameters are summarized in Table~\ref{tab:ggFCR2BDTparameters}.
\begin{table}[h!]
\centering
\begin{tabular}{|l|c|c|}
\hline
...
Six lepton and jet kinematic distributions are used, including $\Delta Y_{\ell\ell}$, $\Delta \phi_{\ell\ell}$, $m_{\ell\ell}$, $m_T$, $\Delta \phi_{jj}$, and $\ensuremath{E_{\text{T}}^{\text{miss}}}$. Distributions for these variables in the ggF CR2 region are shown in Figure~\ref{fig:ggFCR2} and demonstrate MC modeling of data.
\begin{figure}[!h]
  \subfloat[$\Delta Y_{\ell\ell}$]{
...
Figure~\ref{fig:ggFCR3BDTinput} and~\ref{fig:ggFCR3corrSB} demonstrate the input distributions used to train the BDT and their correlations where each distribution is normalized to equal number of background and signal events. 
...
\begin{figure}[!htbp]
    ...
    \caption{Distributions of input variables to the ggF vs. VBF BDT. Samples are normalized to even numbers of background and signal events. Signal represents VBF and background ggF samples~\cite{ourSupportNote}.}
...
\resizebox{\textwidth}{!}{
  ...
  EL\_EFF\_ID\_CorrUncertaintyNP (0 to 15)                &  ID efficiency uncertainty split in 16 components   \\
  EL\_EFF\_ID\_SIMPLIFIED\_UncorrUncertaintyNP (0 to 17)  &  ID efficiency uncertainty split in 18 components   \\
...
The VBF theoretical uncertainties include parton shower, PDF, $\alpha_s$, and QCD scale. Parton shower uncertainties (PS) are derived from comparisons between different parton shower generators (Powheg$+$Pythia8, nominal, and Powheg$+$Herwig7) and constitute the largest theoretical uncertainties in the signal region. The impacts of PS uncertainties are studied at detector-level, and they are based on variations implemented at particle-level which account for model dependency in simulation of the underlying event using the UEEE5 tune~\cite{UEEE5}. PDF uncertainties are derived from internal re-weighting within the nominal Powheg$+$Pythia8 samples. These are evaluated using 68$\%$ confidence level PDF4LHC Hessian eigenvectors added in quadrature~\cite{PDF4LHC15}. QCD uncertainties include factorization and renormalization scales, both independently multiplied by factors of 2.0 and 0.5 relative to the nominal~\cite{HWW2016}. The uncertainties from the strong coupling, $\alpha_s$ , are evaluated by examining variations in the NNPDF3.0 PDF set as $\alpha_s$ is shifted by 1$\sigma$~\cite{PDF4LHC15}. For each value of $\alpha_s$, we produce a set of 100 PDF replicas, whose standard deviation is our measured uncertainty. VBF signal theory systematics within the signal region are shown in Table 6.13.
...
The ggF theoretical uncertainties also include parton shower, PDF, $\alpha_s$, and QCD scale variations. Parton Shower uncertainties use the same prescription described for the VBF samples and similarly constitute the largest theoretical uncertainties from ggF events. PDF uncertainties are derived from internal re-weighting as for VBF and QCD uncertainties include factorization and renormalization scales and are derived using the WG1 scheme. With this scheme, the Higgs $p_T$ uncertainty is derived from the shape change of the Higgs $p_T$ introduced by QCD scale variations~\cite{WG1}. The ggF theory uncertainties derived in the signal region are shown in Table~\ref{tab:ggftheory}.
\begin{table}[h!]
\centering
\scalebox{1.0}{
\input{Pictures/theory_systematics/ggF/table-ggfinvbfdiff.tex}
}
...
We examine these uncertainties for any shape effects on a BDT output trained between $Z+$jets and VBF, described further in Appendix B. Figure~\ref{fig:ztttheor} shows evidence of shape effects beyond those expected from statistical uncertainties, which are large.

\begin{figure}[!h]
  \subfloat[PDF uncertainty]{
      \includegraphics[width=0.4\textwidth]{Pictures/theory_systematics/Ztautau/plots/bdt_Zjets/plots/CutVBFZttVeto_2jet/theo_ztautau_pdf.pdf}
  }\hfill
  \subfloat[$\alpha_s$ uncertainty]{
      \includegraphics[width=0.4\textwidth]{Pictures/theory_systematics/Ztautau/plots/bdt_Zjets/plots/CutVBFZttVeto_2jet/theo_ztautau_alphas.pdf}
  }\hfill
  \subfloat[Generator uncertainty]{
      \includegraphics[width=0.4\textwidth]{Pictures/theory_systematics/Ztautau/plots/bdt_Zjets/plots/CutVBFZttVeto_2jet/theo_ztautau_generator.pdf}
  }\hfill
  \subfloat[QCD scale uncertainty]{
      \includegraphics[width=0.4\textwidth]{Pictures/theory_systematics/Ztautau/plots/bdt_Zjets/plots/CutVBFZttVeto_2jet/theo_ztautau_scale.pdf}
  }\hfill
