...
The VBF theoretical uncertainties include parton shower, PDF, $\alpha_s$, and QCD scale. Shower uncertainties are derived from comparisons between different parton shower generators (Powheg$+$Pythia8, nominal, and Powheg$+$Herwig7) and constitute the largest theoretical uncertainties in the signal region. These are evaluated at particle-level to account for model dependency in simulation of the underlying event using the UEEE5 tune~\cite{UEEE5}. PDF uncertainties are derived from internal re-weighting within the nominal Powheg$+$Pythia8 samples. These are evaluated using $68\%$ confidence level PDF4LHC Hessian eigenvectors added in quadrature~\cite{PDF4LHC15}. QCD uncertainties include factorization (renormalization) scales, independently multiplied by a factor of 2.0 (0.5) relative to the nominal~\cite{Aaboud_2019}. The $\alpha_s$ uncertainties are evaluated by examining differeces between the PDF4LHC15 PDF and NNPDF3.0 PDFs~\cite{PDF4LHC15}. VBF signal theory systematics within the signal region are shown in Table~\ref{tab:vbftheory}.
