...
\caption{Each background and its relative size of in SR (after all cuts), estimation method, and generator is summarized.}
\label{tab:bkgsum}
\end{table}
...
The following plots show kinematic distributions after all preselection cuts with fake factors applied directly after a 2-jet cut is applied. These show good modelling with data over a variety of kinematic variables including those used in the Top$+WW$ vs. VBF BDT described next.
\begin{figure}[!h]
...
The following plots show kinematic distributions after all signal region cuts. Many of these distributions are used to train the 3D BDT that is used in our final fit. Here only MC predictions are shown since data is blinded in the signal region.
\begin{figure}[!h]
  \subfloat[$\Delta Y_{\ell\ell}$]{
